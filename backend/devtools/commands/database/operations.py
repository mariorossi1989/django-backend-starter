...
def drop_database(
    ...
    try:
        ...
        with (
            ...
            console.print(
                f"[yellow]Terminating active connections to '{db_name}'...[/yellow]"
            )
            cursor.execute(
                """
...
def reset_database(
    ...
    try:
        ...
        console.print(
            Panel(
                "[bold cyan]DATABASE RESET[/bold cyan]",
                border_style="cyan",
                expand=False,
            )
        )
        ...
        with connect_to_postgres() as conn, conn.cursor() as cursor:
            # Check if database exists
            cursor.execute("SELECT 1 FROM pg_database WHERE datname = %s", (db_name,))
            db_exists = cursor.fetchone() is not None
