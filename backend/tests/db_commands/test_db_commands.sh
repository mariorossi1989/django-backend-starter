...
test_lifecycle_flow() {
    ...
    print_section "Step 2: Verify database exists"
    print_info "Check your PostgreSQL instance to verify database was created"
    wait_for_user
    
    print_section "Step 3: Drop database"
...
test_reset_flow() {
    ...
    print_section "Testing reset without migrations"
    run_command "Reset (no migrate)" python3 -m devtools db reset --no-migrate || true
    wait_for_user
    
    print_section "Testing reset with migrations"
...
show_menu() {
    ...
    echo "  7) all       - Run all flows sequentially"
    echo "  q) quit      - Exit"
    echo
}
